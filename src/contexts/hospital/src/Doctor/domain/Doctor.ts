...
export class Doctor extends Aggregate {
  ...
  static fromPrimitives(data: Primitives<Doctor>): Doctor {
    return new Doctor(
      ...
      data.educations.map((education: Primitives<Education>) =>
        Education.fromPrimitives(education)
      ),
      data.experiences.map((experience: Primitives<Experience>) =>
        Experience.fromPrimitives(experience)
      ),
